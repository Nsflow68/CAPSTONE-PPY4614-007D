...
export class OllamaService {
  ...
  constructor(
    ...
    this.model = this.configService.get<string>('OLLAMA_MODEL') ?? 'gemma:2b';
  }

  async generate(params: GenerateParams): Promise<GenerateResult> {
    const { prompt, system, temperature = 0.4, timeoutMs = 60000 } = params;
  ...
  async checkHealth(timeoutMs = 2000): Promise<OllamaHealth> {
    const startedAt = Date.now();
    try {
